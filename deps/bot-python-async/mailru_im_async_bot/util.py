...
# Function for rollover file log
def do_rollover_log(signum, frame):
    log.info(f"rollover log by signal {signum}")
    for handler in log.root.handlers:
        if isinstance(handler, DynamicRotatingFileHandler):
            log.info(f"rollover {handler.baseFilename}")
            handler.doRollover()
